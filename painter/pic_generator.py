...
class PicGenerator:
    ...
    def __init__(
        ...
        self._xy = (0, 0)
        self._row_space = 25
        self._bottom_pic: Image.Image | None = None
    ...
    def set_pos(self, x: int | None = None, y: int | None = None) -> "PicGenerator":
        """设置绘图坐标"""
    ...
    def draw_tip(
        self,
        text: str,
        color: Color | tuple[int, int, int] = Color.GRAY,
        xy: tuple[int, int] | None = None
    ) -> "PicGenerator":
        """绘制提示文本"""
        if isinstance(color, Color):
    ...
    def draw_text(
        self,
        texts: str | list[str],
        colors: Color | tuple[int, int, int] | list[Color | tuple[int, int, int]] | None = None,
        xy: tuple[int, int] | None = None
    ) -> "PicGenerator":
        """绘制文本"""
        if colors is None:
