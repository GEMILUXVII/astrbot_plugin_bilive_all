...
class RoomMonitor:
    ...
    def __init__(
        ...
        config: RoomConfig,
        db: StatsDB,
        on_live_start: Callable[["RoomMonitor", dict], Awaitable] | None = None,
        on_live_end: Callable[["RoomMonitor", dict], Awaitable] | None = None,
    ):
        """
        Args:
            config: 房间配置
        ...
        self.room_id: int | None = config.room_id
        self.uname: str | None = config.uname

        self._danmaku: LiveDanmaku | None = None
        self._live_room: LiveRoom | None = None
        self._connecting = False
        self._is_reconnect = False
        self._last_live_push_time: int | None = None
    ...
    async def _handle_live_on(self, event: dict):
        ...
        if self._on_live_start:
            ...
            logger.warning("[BiliLive] 未设置开播回调函数!")
